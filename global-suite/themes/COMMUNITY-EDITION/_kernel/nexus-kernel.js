...
function safeCssAlign(v) {
  const map = { left:'left', center:'center', right:'right', start:'left', end:'right' };
  return map[v] || 'left';
}
...
function detectArchetype(data) {
  ...
  if (!data.recipient && (data.table?.rows?.length || 0) > 5) return 'statement';
  return 'generic_invoice';
}

function applyDensityMode(data) {
  const body = document.body;
  const rowCount = (data.table && data.table.rows) ? data.table.rows.length : 0;
  if (rowCount > THEME_FLAGS.AUTO_DENSITY_LONG_THRESHOLD) body.classList.add('document-long');
  if (rowCount < 4) body.classList.add('a4-short');
  const p = new URLSearchParams(window.location.search);
  const d = p.get('density');
  if (['relaxed','balanced','compact'].includes(d)) {
    body.classList.remove('density-relaxed','density-balanced','density-compact');
    body.classList.add('density-' + d);
  }
  if (p.get('debug') === '1') body.classList.add('debug-mode');
}
...
function renderFault(code) {
  ...
  [
    ...
    ['it-fault-hint',    'Check your Manager.io instance or theme assignment. Restore original file or contact hello@inspiron.tech.']
  ].forEach(([cls, text]) => {
...
function buildCell(cell, col) {
  ...
  if (cell.image && cell.image.url) {
    ...
    if (src) { const img = document.createElement('img'); img.src = src; img.alt = ''; img.style.maxWidth = '100px'; td.appendChild(img); }
  } else if (cell.link && cell.link.url) {
    const href = safeLinkUrl(cell.link.url);
    if (href) { const a = document.createElement('a'); a.href = href; a.target = '_top'; a.className = 'it-cell-link'; appendTextWithBreaks(a, cell.text); td.appendChild(a); }
    else { appendTextWithBreaks(td, cell.text); }
  } else { appendTextWithBreaks(td, cell.text); }
  return td;
}
...
function processRows(rows, columns, tbody) {
  ...
  rows.forEach(r => {
    ...
    tr.className = r.isTotalRow ? 'it-row it-row-subtotal' : 'it-row';
    if (r.cells.length === 1 && r.rows) {
      ...
      processRows(r.rows, columns, tbody); return;
    }
    ...
    if (r.rows) processRows(r.rows, columns, tbody);
  });
}

function markLastRow(tbody) {
  const rows = tbody.querySelectorAll('tr.it-row:not(.it-row-subtotal)');
  if (rows.length > 0) rows[rows.length - 1].classList.add('it-last-row');
}
...
function appendTotals(totals, colCount, tbody) {
  ...
  totals.forEach(total => {
    const tr = document.createElement('tr');
    tr.className = (total.emphasis || total.key === 'Total') ? 'it-total it-total-grand' : 'it-total';
    const lTd = document.createElement('td'); lTd.colSpan = Math.max(colCount - 1, 1); lTd.className = 'it-total-label'; lTd.textContent = safeText(total.label);
    const vTd = document.createElement('td'); vTd.className = 'it-total-value'; vTd.textContent = safeText(total.text);
    tr.appendChild(lTd); tr.appendChild(vTd); tbody.appendChild(tr);
  });
}
...
function setDocumentTitle(data) {
  const parts = [safeText(data.title), safeText(data.reference), safeText(data.business?.name || data.businessName || 'Document')].filter(Boolean);
...
function runIntegrityCheck() {
  ...
  if (declared && expected && declared !== expected) {
    console.warn('[NEXUS] Theme version mismatch. Declared: ' + declared + ' · Expected: ' + expected + '. Restore original file or contact hello@inspiron.tech.');
  }
}
...
function renderDebugOverlay(meta) {
  ...
  ['— NEXUS DEBUG —', 'Theme: ' + safeText(meta.themeId), 'Version: ' + safeText(meta.themeVersion), 'Archetype: ' + safeText(meta.archetype), 'Rows: ' + safeText(meta.rowCount), 'Schema: /api4/view-v1'].forEach(line => {
    const p = document.createElement('p'); p.textContent = line; overlay.appendChild(p);
  });
